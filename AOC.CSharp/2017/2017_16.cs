...
public static class AOC2017_16
{
    ...
    {
        List<string> programs = Enumerable.Range(0, programCount).Select(x => ((char)('a' + x)).ToString()).ToList();
    ...
    public static string Solve2(string[] lines, int programCount)
    {
        ...
        // Dance until we see a result repeat. This tells us how many iterations need to be done before we start
        // cycling through the same results
        int repeatNum = 0;
        ...
        // Now that we know the repeat frequency, we can eliminate most of the iterations by removing all the
        // repetitive cycles until just before the target iteration. Once we get close, we can continue dancing
        // again in a loop until we hit the exact target we are aiming for.
        int newStart = (1000000000 / repeatNum) * repeatNum;
        programs = Enumerable.Range(0, programCount).Select(x => ((char)('a' + x)).ToString()).ToList();
