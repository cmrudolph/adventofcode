...
module AOC2015_21 =
            ...
            for a in armor do
                for i in [0..rings.Length - 1] do
                    for j in [i + 1..rings.Length - 1] do
                        ...
                        results.Add({ PlayerWon = finalPlayerHealth > 0; Gold = loadout.Cost })
    ...
    let solve1 (lines : string[]) =
        ...
        results.Where((fun r -> r.PlayerWon)).Select((fun r -> r.Gold)).Min() |> int64

    let solve2 (lines : string[]) =
        let boss = parse lines
        let results = simulateAll boss

        results.Where((fun r -> (not r.PlayerWon))).Select((fun r -> r.Gold)).Max() |> int64