...
module AOC2015_07 =
    ...
    type Instruction = { Target: Wire; Action: Action }
    ...
    let parse line =
        ...
        let parseAnd =
            ...
            if m.Success then
                Some
                    { Target = wire 3 m
                      Action = BitwiseAnd(either 1 m, either 2 m) }
            else
                None
        ...
        let parseOr =
            ...
            if m.Success then
                Some
                    { Target = wire 3 m
                      Action = BitwiseOr(either 1 m, either 2 m) }
            else
                None
        ...
        let parseLeftShift =
            ...
            if m.Success then
                Some
                    { Target = wire 3 m
                      Action = LeftShift(wire 1 m, value 2 m) }
            else
                None
        ...
        let parseRightShift =
            ...
            if m.Success then
                Some
                    { Target = wire 3 m
                      Action = RightShift(wire 1 m, value 2 m) }
            else
                None
        ...
        let parseNot =
            ...
            if m.Success then
                Some { Target = wire 2 m; Action = Negate(wire 1 m) }
            else
                None
        ...
        let parseAssign =
            ...
            if m.Success then
                Some { Target = wire 2 m; Action = Assign(either 1 m) }
            else
                None
    ...
    let rec calc
        (cache: Dictionary<Wire, int>)
        (inst: Map<Wire, Instruction>)
        targetWire
        =
    ...
    let solve2 (lines: string []) =
        ...
        let inst2 =
            Map.change
                b
                (fun _ -> Some { Target = b; Action = Assign(V a1) })
                inst1
