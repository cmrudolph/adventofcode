...
module AOC2015_06 =
    ...
    let grpStr (m: Match) (idx: int) = m.Groups.[idx].Value

    let grpInt (m: Match) (idx: int) = grpStr m idx |> int
    ...
    let sumOn (grid: int [,]) = grid |> Seq.cast<int> |> Seq.sum |> int64
    ...
    let solveCase apply lines =
        ...
        |> Array.map
            (fun m ->
                (grpStr m 1, (grpInt m 2, grpInt m 3, grpInt m 4, grpInt m 5)))
