...
module AOC2020_10 =
    let updateAt targetIdx func lst =
        lst
        |> List.mapi
            (fun idx value -> if idx = targetIdx then (func value) else value)
    ...
    let parseAndAugment (lines: string []) =
        let nums =
            lines |> List.ofArray |> List.map System.Int64.Parse |> List.sort
    ...
    let solve2Impl nums =
        ...
        let finalPaths =
            (pathMap, tail)
            ||> List.fold (fun acc x -> acc |> Map.add x (numPathsTo acc x))
