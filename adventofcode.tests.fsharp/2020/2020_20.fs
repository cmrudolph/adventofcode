...
module Day20 =
    ...
    let getSides (contents : char[,]) =
        ...
        let right = new string([|1..len|] |> Array.map (fun i -> flat.[(i*len)-1]))
        let bottom = new string(flat.[(len*(len-1))..])
        let left = new string([|0..len-1|] |> Array.map (fun i -> flat.[i*len]))
        [
    ...
    let parseTile (cleanLine : string) =
        ...
        { Number = number; Contents = arr2d; Top = sides.[0]; Right = sides.[1]; Bottom = sides.[2]; Left = sides.[3] }
    ...
    let edgeifyTile allTiles tile =
        ...
        { tile with Top = newTop; Right = newRight; Bottom = newBottom; Left = newLeft }

    let edgeifyTiles tiles =
        tiles
        |> List.map (edgeifyTile tiles)
    ...
    let stitchItTogether tiles =
        ...
        let mutable remaining = tiles |> List.filter (fun tile -> tile.Number <> topLeft.Number)
        ...
        for _ in [1..len-1] do
            ...
            remaining <- remaining |> List.filter (fun tile -> tile.Number <> next.Number)
            curr <- next

        // Next N rows
        for i in [1..len-1] do
            // Leftmost in new row
            let next = findWithTop ordered.[(i-1)*len].Bottom.Value remaining
            ordered <- List.append ordered [next]
            remaining <- remaining |> List.filter (fun tile -> tile.Number <> next.Number)
            curr <- next
            ...
            for j in [1..len-1] do
                ...
                let next = findWithTopAndLeft targetTop curr.Right.Value remaining
                ordered <- List.append ordered [next]
                remaining <- remaining |> List.filter (fun tile -> tile.Number <> next.Number)
                curr <- next
    ...
    let buildCombinedImage (tiles : Tile list) =
        ...
        for i in [0..totalLen-1] do
            let shouldProcessRow = ((i % contentLen) <> 0) && ((i % contentLen) <> contentLen-1)
            if shouldProcessRow then
                let mutable chars = ([] : TrackedChar list)
                for j in [0..totalLen-1] do
                    let shouldProcessCol = ((j % contentLen) <> 0) && ((j % contentLen) <> contentLen-1)
                    if shouldProcessCol then
                        let tileIdx = ((i/contentLen) * tileLen) + (j/contentLen)
                        let x = i % contentLen
                        let y = j % contentLen
                        let tracked = { Id = counter; CharValue = tiles.[tileIdx].Contents.[x,y] }
                        counter <- counter + 1
    ...
    let getSeaMonsterIds arr =
        ...
        for i in [0..len-3] do
            for j in [0..len-20] do
                ...
                if areMonsterIds then allMonsterIds <- Set.union allMonsterIds (ids |> Set.ofList)
    ...
    [<Fact>]
    let Sample () =
        readInput "2020" "20" "sample" |> solveAndValidate (20899048083289L, 273L) solve

    [<Fact>]
    let Actual () =
        readInput "2020" "20" "actual" |> solveAndValidate (83775126454273L, 1993L) solve